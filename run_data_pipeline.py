...
What this script does, end to end:
  ...
  6. Snaps crime, stop and search, and lamp points onto the nearest street
     edge, then builds TWO separate edge level feature tables:
       - edge_features.csv/.gpkg: crime + stop and search features, both as
         an overall total across the whole resolved date range AND broken
         out per month as separate columns.
       - edge_lamp_features.csv/.gpkg: lamp features (lamp_count,
         lamp_per_km, is_lit). Kept in its own file on purpose, lighting is
         a live snapshot with no time dimension, it is a different kind of
         feature to crime/stop-search and mixing it into the same table
         made it harder to see what was what.
...
Notes on data.police.uk, checked directly against their API docs:
  ...
  - Crime grade / seriousness: NOT provided by the API, there is no
    ...
    comment above that constant for the citations.
  - Crime by day / exact date: NOT available for crimes, only "month"
    (YYYY-MM), that is a hard limit of the crimes-street endpoint. This is
    also why the monthly breakdown below is by month, not by day.
  - Stop and search DOES carry a real datetime (down to the minute in most
    forces), see fetch_stop_search() below, but is still only split by
    month at the edge feature level, for consistency with crime.
  - Date range: the API only keeps a rolling window of roughly the last
    ...
    note if it had to. The "total" columns (crime_count, crime_severity_sum,
    stop_search_count) are summed across that ENTIRE resolved range, not
    just one year, main() prints exactly which months went into that sum.
...
def get_network(place: str, network_type: str):
    ...
    GIS-tool readable on their own. These are the ONLY coordinate columns
    on the edge table itself, lamp coordinates never touch this table, see
    build_lamp_features() further down.

    NOTE: 'lit' is an OSM tag that is only present on edges where someone
    actually mapped it. If NO edge in the whole download has that tag (this
    happens, Camden's OSM data turned out to have none), the column will
    not exist at all afterwards, not just be empty. Every downstream use of
    "lit" has to check for that with `if "lit" in ...` rather than assuming
    ...
    if "lit" not in edges.columns:
        print("[network] note: no edge in this download has an OSM 'lit' tag, "
              "the is_lit column in edge_lamp_features will rely entirely on "
              "whether a Camden lamp got snapped to that edge.")
...
def fetch_stop_search(boundary_poly_metric, months: list[str]) -> gpd.GeoDataFrame:
    ...
    anything about the street itself.
    ...
    exact times with some caution). A "month" column is still derived from
    it for consistency with crime when splitting by month.
    """
    poly_wgs = (gpd.GeoSeries([boundary_poly_metric], crs=METRIC_CRS)
                .to_crs(epsg=WGS84).iloc[0])
    poly_str = _poly_param(poly_wgs)
...
def fetch_lighting(url: str) -> gpd.GeoDataFrame:
    ...
    fires and simplify if you like. This is a live snapshot of current
    lamp assets, there is no month/date dimension to split by here.
    """
...
def snap_counts_by_month(points: gpd.GeoDataFrame, edges: gpd.GeoDataFrame,
    ...
    """Same idea as snap_counts, but pivoted into one column per month.

    Column names look like crime_count_2024_01 (hyphens swapped for
    underscores so they are safe to use as plain Python/pandas identifiers).
    Returns a DataFrame indexed by edge_id, ready to merge onto the main
    feature table. If points has no 'month' column, or is empty, returns an
    empty frame.
    """
...
def build_crime_stop_features(edges: gpd.GeoDataFrame, crime: gpd.GeoDataFrame,
    ...
    """Crime and stop-search features only, NOT lamps.

    Kept separate from lamp features on purpose (see build_lamp_features),
    so this table only ever carries the street network's own coordinates
    (u_lat/u_lng/v_lat/v_lng, inherited from `edges`) plus crime/stop-search
    counts, never lamp coordinates.

    Includes both an overall total per edge (crime_count, crime_severity_sum,
    stop_search_count, summed across the ENTIRE resolved date range) and a
    per-month breakdown as separate columns (crime_count_2024_01, etc).
    """
...
def build_lamp_features(edges: gpd.GeoDataFrame, lamps: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """Lamp features only, kept in a table of its own.

    Lighting is a live snapshot (no time dimension, unlike crime/stop-search)
    and is a different kind of feature entirely, so it gets its own file
    (edge_lamp_features.csv/.gpkg) rather than being merged into
    edge_features.csv. Only edge_id, the street's own length/lit tag (if
    present, see the note in get_network about 'lit' sometimes not existing
    at all), and the derived lamp columns are kept here, no lamp coordinates
    end up anywhere near the street network's own u/v coordinates.
    """
    lamp_count = snap_counts(lamps, edges, LAMP_SNAP_MAX_M, "lamp_count")

    # 'lit' may not exist at all if no edge in this download had that OSM
    # tag, guard the column selection the same way build_crime_stop_features
    # implicitly does via edges itself, do not assume it is always there.
...
def main():
    ...
    # crime + stop-search features, WITH monthly breakdown, no lamp columns
    ...
    # lamp features, in their own separate file
    ...
    print(f"[summary] totals below are summed across ALL resolved months "
    ...
    print(f"[summary] {len(monthly_cols)} monthly breakdown columns were also added to "
          f"edge_features.csv, e.g.:")
