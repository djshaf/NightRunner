...
## Two Kinds of Smoothing, and Why They Differ

The nearest-edge counts above snap each crime to one single street. That is a hard, all or nothing choice, so `edge_features.csv` also spreads each crime out over nearby streets in two different ways. They sound similar but do different jobs, and mixing them up would cause a problem, so they are kept apart on purpose.

The first is a straight-line radius view, kept for description only. For four radii (50m, 75m, 125m, 150m), every crime or stop and search within that straight-line distance of a street contributes to it, weighted down to zero at the radius edge. This is a simplified Kernel Density Estimation (KDE), the standard technique in crime hotspot mapping (Chainey, Tompson and Uhlig, 2008). 125m matches our own estimate of how far apart data.police.uk's anonymised points sit in Camden. Columns: `crime_density_r{R}` / `stop_search_density_r{R}` (weighted count), and `severity_wavg_r{R}` / `perceived_risk_wavg_r{R}` (distance-weighted average). Two things make this view unfit as a model input: it measures straight-line distance, so it can smooth a crime through a wall, a back garden or across the canal to a street you cannot actually reach; and it is computed once over the whole date range, not per month. For a time series model that second point matters most, pooling all months first would let a crime from 2026 bleed into a street's 2023 figures and leak the future into the past. So these radius columns stay as a human-facing "what does my area look like" description, and are not fed to the model.

The second is Network KDE (NetKDE), and this is the version meant for the model. It works the same way, close crimes count for more, but distance is measured along the walkable street graph using Dijkstra shortest paths, not in a straight line, so a crime spreads down real streets a runner could actually take. Just as important, it is applied separately inside each month, so no future month can leak backwards. The columns are named `{metric}_netkde_{year}_{month}`, for example `crime_count_netkde_2024_01`, matching the monthly breakdown naming so the downstream reshape picks them up. Bandwidth defaults to 125m, the same spacing figure as above; change `NETKDE_BANDWIDTH_M` in the script to adjust it. This smoothing was moved into the data pipeline from the ML notebook so it runs once here, and so the street graph is built once rather than rebuilt for every month.
