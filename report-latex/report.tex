...
\textbf{<PLACEHOLDER - re. Data>} Several limitations constrain the data pipeline. Crime records are month-level, so the signal cannot separate night from day despite night safety being the core concern. Two weightings were built, but only perceived risk feeds the model. Stop-and-search counts reflect where police operate, not solely where danger lies, and \texttt{is\_lit} only flags a lamp within 25m, with no data on whether it works and the same snapshot applied to every month. Finally, the study covers Camden alone, so generalisation to other areas is untested.
...
Note: For the rubric, refer to the Group Project Guidelines: \url{https://qmplus.qmul.ac.uk/mod/resource/view.php?id=3794535}
