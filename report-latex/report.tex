...
After procesing the data is passed through an XGBoost Regressor using spatial-temporal lags and infrastructure metadata received to forecast street-level risk (percieved). Model employs tweedie deviance for zero-inflated, right-skewed distribution seen in this localization. These continuous values are then scaled and integrated with lighting penalties, to be fed into the pathfinding algorithm for optimal, safe routing. 
...
The objective of this predictive safety routing system is to compute an optimal and safe path for pedestrians and runners operating at night. Urban street network is modeled as a weighted directed graph $G = (V, E)$, where $V$ represents street intersections (nodes), $E$ represents street segments (edges) ~\cite{LozanoDominguez2021}. Each edge $e = (u, v) \in E$ has a street length $l_e$, spatial geometry, static street infrastructure, and time-series historical crime metrics aggregated at monthly intervals $t \in T$. 

The system separates future crime risk prediction from graph pathfinding by utilizing an \textbf{XGBoost Regressor} that estimates future spatial-temporal risk $y_{e, t}$ per street segment. These are then transformed to normalized edge costs $C_e \in [1, 10]$ and integrated into a dynamic graph function that accounts for environmental factors to determine optimal path via Djistra search algorithm. 

\subsubsection{Network Kernel Density Estimation (NetKDE)}

NetKDE was chosen in order to model risk diffusion along actual pedestrian pathways rather than euclidian space, while eliminating artifacts caused by spatial snapping~\cite{Wu2022}. For each incident $i$ on edge $e$ risk diffuses in walkable paths using Djikstra's shortest-path distance $d_{\text{net}}(e, e')$ with a Gaussian kernel function $K(d_{\text{net}})$ bounded by distance $B = 125\text{ m}$. 
\begin{align*}
...
As UK police API is subject to a 2-month lag, predictive feature strictly omits real-time figures. Input feature vector $\mathbf{x}_{e, t}$ for edge $e$ at prediction time $t$ is as follows: 
...
\begin{enumerate}
    ...
    \item Native support for categorical features via numerical splits without expanding feature dimentionality.
    \item Fast prediction latency enabling faster scoring if more frequent data was to be obtained. 
\end{enumerate}
...
Night-time routing introduces another objective optimization problem, due to the dark penalty ($\delta = 5.0$) the night time routing achieved a 53.35\% reduction in darkness exposure . However, this comes at a cost of a detour penalty jumping to 23.15\%, and a crime risk reduction metric dropping from 80.46\% to a 63.72\%(see Appendix \ref{app:data_tables}). This drop makes logical sense, as well-lit streets (commercial and nightlife districts) may sometimes suffer from higher base crime rates. The risk reduction is still very sizeable, and this represents a compromise to ensure that routing is significantly safer in a both percieved and physical sense. 
