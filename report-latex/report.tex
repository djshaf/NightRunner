...
\begin{abstract}
   ...
   \textbf{<PLACEHOLDER - to be written last>}
\end{abstract}
...
\textbf{<PLACEHOLDER - Daniel to review/edit>}
A routable pedestrian street graph for Camden is obtained from OpenStreetMap via OSMnx~\cite{boeing2017osmnx}. Three categories of open data are attached to this graph at street (edge) level: recorded crime and stop-and-search incidents from the UK Police open data API~\cite{datapoliceuk2026}, and street lighting locations from Camden Council's open data portal~\cite{camden2026lighting}. Data is requested by month; because data.police.uk only retains a rolling window of recent history, the pipeline detects the earliest month actually available at run time and clips the request accordingly rather than assuming a fixed start date.

Two harm-oriented crime weightings are computed per incident: a \emph{severity} score (1-3) based on sentencing-severity weightings from the Cambridge Crime Harm Index~\cite{sherman2016cambridge} and the ONS Crime Severity Score methodology~\cite{ons2016severity}, and a separate \emph{perceived risk} score (1-4) based on the Signal Crimes Perspective~\cite{innes2004signal,innesfielding2002signal} and ONS perception/anti-social-behaviour survey data~\cite{ons2022csew}, under which visible disorder (e.g. anti-social behaviour) can score above a more legally severe but less visible offence. Both are acknowledged simplifications of each source study's original weights, applied onto data.police.uk's 14 broad offence categories rather than a reproduction of either study.

Because police-recorded incident coordinates are anonymised to a fixed set of snap points rather than exact addresses, each incident's influence is spread across nearby streets using Network Kernel Density Estimation (NetKDE)~\cite{chainey2008hotspot}, with distance measured along the walkable street graph (via Dijkstra shortest paths) rather than in a straight line, so an incident cannot influence a street on the other side of a wall or a canal. A spatial lag is also computed, aggregating each metric across a street's directly neighbouring edges. Lighting features (lamp count, lamp density per kilometre, and a binary lit/unlit flag) are collected separately and are not subject to this same temporal or predictive treatment (see \emph{Lighting Indicator} below).
...
\subsection{Dataset}
The street network, crime, stop-and-search, and lighting data are all scoped to the London Borough of Camden, extracted as an OpenStreetMap \texttt{.osm.pbf} file via the BBBike extract service~\cite{bbbike2026}. \textbf{The exact date range of data actually used for model training, and the final row counts after cleaning, are not recorded in the materials available for this report and should be confirmed and stated here by the data lead before submission.} Records with missing or non-numeric coordinates and exact duplicate rows are removed during cleaning, and all coordinates are reprojected to the British National Grid to support metre-based distance calculations.
...
\begin{figure}[t]
\begin{center}
   \includegraphics[width=0.8\linewidth]{figures/ui-screenshot-1.pdf}
\end{center}
...
\begin{figure}[t]
\begin{center}
   \includegraphics[width=0.8\linewidth]{figures/ui-screenshot-2.pdf}
\end{center}
...
\begin{itemize}
\item \textbf{AI Research \& Data Lead - Hongyuan Zhai:} AI problem formulation, requirement analysis, and data collection, cleaning, and preparation.
\item \textbf{AI Model Lead - David Shafirovich:} AI model implementation, training, testing, and evaluation.
